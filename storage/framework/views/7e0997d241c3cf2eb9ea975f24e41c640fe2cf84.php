
<body class="g2Bg">
<section id="main">
    <div id="loading">
        <div class="loader">
            <div class="loader-inner pacman">
                <div></div>
                <div></div>
                <div></div>
                <div></div>
                <div></div>
            </div>
            <p> 資源加載中... <br> <span>1</span>%</p>
        </div>

    </div>

    <div id="g2-park1">
        <div class="tips">
            <div class="tips-item">
                <p class="gcenterIn" style="width: 50.7%"><img src="<?php echo theme_asset_lang("images/g2-1.png"); ?>" alt=""></p>
                <br/>
                <div class="btn g2-park1-btn delay-1 gdownIn" style="width: 19.2%"><img src="<?php echo theme_asset_lang("images/g2-2.png"); ?>" alt=""></div>
            </div>
        </div>
    </div>
    <div id="g2-park2">
        <div id="video_box" >
            <video id="g2-park2-video" class="video" style="object-fit:fill;"  width="100%" htyle="object-fit:fill;"  width="100%" height="100%"  preload="auto" playsinline="true" webkit-playsinline="true" x-webkit-airplay="allow" airplay="allow"  x5-video-orientation="portrait" x5-video-player-type="h5" x5-video-player-fullscreen="true" src="<?php echo theme_asset_lang("video/g2-park2.mp4"); ?>"></video>
        </div>
    </div>
    <div id="g2-park3">
        <div class="question" >
            <div class="question-bg" style="width: 60%">
                <img src="<?php echo theme_asset_lang("images/g2-3.png"); ?>" width="100%" alt="">
                <div class="question-list">
                    <div class="question-item fb-transition grightIn" tid = "A" option_id="1" question_id="1">
                        <img src="<?php echo theme_asset_lang("images/g2-4.png"); ?>" width="100%" alt="">
                    </div>
                    <div class="question-item fb-transition grightIn"  tid = "B" option_id="2" question_id="1">
                        <img src="<?php echo theme_asset_lang("images/g2-5.png"); ?>" width="100%" alt="">
                    </div>
                    <div class="question-item fb-transition grightIn"  tid = "C" option_id="3" question_id="1">
                        <img src="<?php echo theme_asset_lang("images/g2-6.png"); ?>" width="100%" alt="">
                    </div>
                    <div class="question-item fb-transition grightIn"  tid = "D" option_id="4" question_id="1">
                        <img src="<?php echo theme_asset_lang("images/g2-7.png"); ?>" width="100%" alt="">
                    </div>
                </div>
                <div class="question-power2 gdownIn fb-clearfix">
                    <div class="question-home bbtn"><a href="index.html"><img src="<?php echo theme_asset_lang("images/home.png"); ?>" alt=""></a></div>
                    <div class="question-reset bbtn"><img src="<?php echo theme_asset_lang("images/reset.png"); ?>" alt=""></div>
                    <div class="question-last bbtn"><img src="<?php echo theme_asset_lang("images/last.png"); ?>" alt=""></div>
                </div>
            </div>
            <input type="hidden" name="g2-park3Answer" value="">
            <input type="hidden" name="g2-park3Answer-option-id" value="">
        </div>

    </div>
    <div id="g2-park3-1">
        <div class="tips">
            <div class="tips-item">
                <p class="gcenterIn" style="width: 50.7%"><img src="<?php echo theme_asset_lang("images/g2-8-1.png"); ?>" alt=""></p>
                <br/>
                <div class="question-power3  fb-clearfix delay-1 gdownIn">
                    <div class="question-home bbtn"><a href="index.html"><img src="<?php echo theme_asset_lang("images/home.png"); ?>" alt=""></a></div>
                    <div class="question-reset bbtn"><img src="<?php echo theme_asset_lang("images/reset.png"); ?>" alt=""></div>
                    <div class="question-last bbtn"><img src="<?php echo theme_asset_lang("images/last.png"); ?>" alt=""></div>
                </div>
            </div>
        </div>
    </div>
    <div id="g2-park3-2">
        <div class="tips">
            <div class="tips-item">
                <p class="gcenterIn" style="width: 50.7%"><img src="<?php echo theme_asset_lang("images/g2-8-2.png"); ?>" alt=""></p>
                <br/>
                <div class="question-power3  fb-clearfix delay-1 gdownIn">
                    <div class="question-home bbtn"><a href="index.html"><img src="<?php echo theme_asset_lang("images/home.png"); ?>" alt=""></a></div>
                    <div class="question-reset bbtn"><img src="<?php echo theme_asset_lang("images/reset.png"); ?>" alt=""></div>
                    <div class="question-last bbtn"><img src="<?php echo theme_asset_lang("images/last.png"); ?>" alt=""></div>
                </div>
            </div>
        </div>
    </div>
    <div id="g2-park4">
        <div id="video_box-park4" >
            <video id="g2-park4-video" class="video" style="object-fit:fill;"  width="100%" htyle="object-fit:fill;"  width="100%" height="100%"  preload="auto" playsinline="true" webkit-playsinline="true" x-webkit-airplay="allow" airplay="allow"  x5-video-orientation="portrait" x5-video-player-type="h5" x5-video-player-fullscreen="true" src="<?php echo theme_asset_lang("video/g2-park4.mp4"); ?>"></video>
        </div>
    </div>
    <div id="g2-park5">
        <div class="tips">
            <div class="tips-item">
                <p class="gcenterIn delay-1" style="width: 56%"><img src="<?php echo theme_asset_lang("images/g2-9.png"); ?>" alt=""></p>
                <br>
                <p class="gcenterIn delay-2" style="width: 56%"><img src="<?php echo theme_asset_lang("images/g2-10.png"); ?>" alt=""></p>
                <br/>
                <div class="question-power3  fb-clearfix delay-1 gdownIn">
                    <div class="question-home bbtn"><a href="index.html"><img src="<?php echo theme_asset_lang("images/home.png"); ?>" alt=""></a></div>
                    <div class="question-reset bbtn"><img src="<?php echo theme_asset_lang("images/reset.png"); ?>" alt=""></div>
                    <div class="question-last bbtn"><img src="<?php echo theme_asset_lang("images/last.png"); ?>" alt=""></div>
                </div>
            </div>
        </div>
    </div>
    <div id="g2-park6">
        <div class="tips">
            <div class="tips-item">
                <p class="gcenterIn delay-1" style="width: 71.1%"><img src="<?php echo theme_asset_lang("images/g2-11.png"); ?>" width="100%" alt=""></p>
                <br>
                <p class="gcenterIn delay-2" style="width: 30.4%"><img src="<?php echo theme_asset_lang("images/g2-12.png"); ?>" width="100%" alt=""></p>
                <br/>
                <div class="question-power3  fb-clearfix delay-1 gdownIn" >
                    <div class="question-home bbtn"><a href="index.html"><img src="<?php echo theme_asset_lang("images/home.png"); ?>" alt=""></a></div>
                    <div class="question-reset bbtn"><img src="<?php echo theme_asset_lang("images/reset.png"); ?>" alt=""></div>
                    <div class="question-last bbtn"><img src="<?php echo theme_asset_lang("images/last.png"); ?>" alt=""></div>
                </div>
            </div>
        </div>
    </div>
    <div id="g2-park7">
        <div id="video_box-park7" >
            <video id="g2-park7-video" class="video" style="object-fit:fill;"  width="100%" htyle="object-fit:fill;"  width="100%" height="100%"  preload="auto" playsinline="true" webkit-playsinline="true" x-webkit-airplay="allow" airplay="allow"  x5-video-orientation="portrait" x5-video-player-type="h5" x5-video-player-fullscreen="true" src="<?php echo theme_asset_lang("video/g2-park6.mp4"); ?>"></video>
        </div>
    </div>
    <div id="g2-park8">
        <div class="tips">
            <div class="tips-item">
                <p class="gcenterIn delay-1" style="width: 64.3%"><img src="<?php echo theme_asset_lang("images/g2-13.png"); ?>" width="100%" alt=""></p>
                <br>
                <p class="gcenterIn delay-2" style="width: 64.3%;position: relative;">
                    <img src="<?php echo theme_asset_lang("images/g2-14.png"); ?>" alt="">

                    <textarea  id="" cols="30" rows="10" name="g2-park8Answer" question_id="2"></textarea>

                </p>
                <br/>

                <div class="question-power3  fb-clearfix delay-1 gdownIn">
                    <div class="question-home bbtn"><a href="index.html"><img src="<?php echo theme_asset_lang("images/home.png"); ?>" alt=""></a></div>
                    <div class="question-reset bbtn"><img src="<?php echo theme_asset_lang("images/reset.png"); ?>" alt=""></div>
                    <div class="question-last bbtn"><img src="<?php echo theme_asset_lang("images/last.png"); ?>" alt=""></div>
                </div>
            </div>
        </div>
    </div>
    <div id="g2-park9">
        <div class="tips">
            <div class="tips-item">
                <div class="g2-result gcenterIn" style="width: 26.2%" ><img src="<?php echo theme_asset_lang("images/g2-result.png"); ?>" width="100%" alt=""></div>
                <br>

                <div class="question-power3  fb-clearfix  delay-1 gdownIn" style="margin-top: 0">
                    <div class="question-home bbtn"><a href="index.html"><img src="<?php echo theme_asset_lang("images/home.png"); ?>" alt=""></a></div>
                    <div class="question-reset bbtn"><img src="<?php echo theme_asset_lang("images/reset.png"); ?>" alt=""></div>
                    <div class="question-last bbtn"><img src="<?php echo theme_asset_lang("images/last.png"); ?>" alt=""></div>
                </div>
            </div>
        </div>
    </div>
    <!-- <div id="video_box" style="opacity: 0">
      <video id="byl_video" style="object-fit:fill;"  width="100%" htyle="object-fit:fill;"  width="100%" height="100%"  preload="auto" playsinline="true" webkit-playsinline="true" x-webkit-airplay="allow" airplay="allow"  x5-video-orientation="portrait" x5-video-player-type="h5" x5-video-player-fullscreen="true" src="<?php echo theme_asset_lang("video/2.mp4"); ?>"></video>
    </div>

    <canvas class="dynaLayer__canvas video_welcome" width="1920" height="1080" id="canvas"></canvas> -->

</section>


</body>
<script>
    // g2Park1In();
    function hideLaoding(){
        $("#loading").hide()
    }
    function g2Park1In(){
        $("#g2-park1").fadeIn()
    }
    function g2Park1Out(){
        $("#g2-park1").hide()
    }
    function g2Park2In(){
        $("#g2-park2").fadeIn();


    }
    function g2Park2Out(){
        $("#g2-park2").hide();

    }
    function g2Park3In(){
        $("#g2-park3").fadeIn();

    }
    function g2Park3Out(){
        $("#g2-park3").hide();

    }
    function g2Park3_1In(){
        $("#g2-park3-1").fadeIn();

    }
    function g2Park3_1Out(){
        $("#g2-park3-1").hide();

    }
    function g2Park3_2In(){
        $("#g2-park3-2").fadeIn();

    }
    function g2Park3_2Out(){
        $("#g2-park3-2").hide();
    }
    function g2Park4In(){
        $("#g2-park4").fadeIn();
    }
    function g2Park4Out(){
        $("#g2-park4").hide();

    }
    function g2Park5In(){
        $("#g2-park5").fadeIn();

    }
    function g2Park5Out(){
        $("#g2-park5").fadeOut();

    }
    function g2Park6In(){
        $("#g2-park6").fadeIn();

    }
    function g2Park6Out(){
        $("#g2-park6").hide();

    }
    function g2Park7In(){
        $("#g2-park7").fadeIn();

    }
    function g2Park7Out(){
        $("#g2-park7").hide();

    }
    function g2Park8In(){
        $("#g2-park8").fadeIn();

    }
    function g2Park8Out(){
        $("#g2-park8").hide();

    }
    function g2Park9In(){
        $("#g2-park9").fadeIn();

    }
    function g2Park9Out(){
        $("#g2-park9").hide();

    }

    var timer = null;
    var n = 0;
    loadingNum();
    function loadingNum(){
        timer = setInterval(function(){
            var n = parseInt($("#loading p span").text());
            n = ++n > 99 ? 99 : n;
            $("#loading p span").text(n)
        },500)
    }
    // 判断视频是否完全加载完毕
    function go(){
        var count = $(".video").length;
        ++n;
        if( n == count){
            //  开始
            clearInterval(timer);
            hideLaoding();
            g2Park1In();
        }
    }
    // 判断视频是否完全加载完毕
    //视频卡顿提醒
    function videoWaiting(){
        fbAlert("網絡有點卡頓，正在緩衝中。結束後可點左下角重新播放視頻哦",3000)
    }
    //视频卡顿提醒
    //第二部 s
    videoElem = document.getElementById('g2-park2-video');
    videoElem.addEventListener('canplaythrough',cacheFun1);
    videoElem.addEventListener('ended',function () {
        g2Park2Out();
        g2Park3In();
        videoElem.removeEventListener('waiting',videoWaiting)
    });
    videoElem.addEventListener('waiting',videoWaiting)
    function cacheFun1(){
        go();
        $(".g2-park1-btn").fadeIn(200)
        $(".g2-park1-btn").on("click",function(){
            g2Park1Out()
            g2Park2In()
            videoElem.play()

        })
        videoElem.removeEventListener("canplaythrough",cacheFun1);
    }
    //第二部 e
    //第三步 s

    $("#g2-park3 .question-item").on("click",function(){
        var n = $(this).attr("tid");
        var option_id = $(this).attr("option_id");
        $("[name='g2-park3Answer']").val(n);
        $("[name='g2-park3Answer-option-id']").val(option_id);
        //提交答案
        $(this).addClass("active").siblings(".question-item").removeClass("active")
    })
    $("#g2-park3 .question-reset").on("click",function(){
        //重播
        g2Park2In();
        g2Park3Out();
        videoElem.play()
    })
    //第三步 e
    //第四步 s
    videoElem2 = document.getElementById('g2-park4-video');
    videoElem2.addEventListener('canplaythrough',cacheFun2);
    videoElem2.addEventListener('ended',function () {
        g2Park4Out();
        g2Park5In();
        videoElem2.removeEventListener('waiting',videoWaiting)
    });
    videoElem2.addEventListener('waiting',videoWaiting)
    function cacheFun2(){
        go();
        $("#g2-park3 .question-last").on("click",function(){
            var p3answer = $("[name='g2-park3Answer']").val();
            var p3answer_option_id = $("[name='g2-park3Answer-option-id']").val();
            if(p3answer.length == 0){
                fbAlert("請先選擇答案");
                return false;
            }
            if(p3answer =="A" || p3answer =="B" || p3answer =="C"  ){
                g2Park3Out()
                g2Park3_1In()
            }else if(p3answer == "D"){
                g2Park3Out()
                g2Park3_2In()
            }else{
                fbAlert("請先選擇答案");
            }
            submit_option(p3answer_option_id);
        })
        videoElem2.removeEventListener("canplaythrough",cacheFun2);
    }
    $("#g2-park3-1 .question-reset").on("click",function(){
        //重播
        g2Park2In();
        g2Park3_1Out();
        videoElem.play()
    })
    $("#g2-park3-2 .question-reset").on("click",function(){
        //重播
        g2Park2In();
        g2Park3_2Out();
        videoElem.play()
    })
    $("#g2-park3-1 .question-last").on("click",function(){
        g2Park4In();
        g2Park3_1Out();
        videoElem2.play()
    })
    $("#g2-park3-2 .question-last").on("click",function(){
        g2Park4In();
        g2Park3_2Out();
        videoElem2.play()
    })
    //第四步 e
    //第五步 s
    $("#g2-park5 .question-reset").on("click",function(){
        g2Park4In();
        g2Park5Out();
        videoElem2.play()
    })
    $("#g2-park6 .question-reset").on("click",function(){
        g2Park4In();
        g2Park6Out();
        videoElem2.play()
    })
    $("#g2-park5 .question-last").on("click",function(){
        g2Park5Out();
        g2Park6In();

    })
    //第五步 e
    //第六步 s
    videoElem3 = document.getElementById('g2-park7-video');
    videoElem3.addEventListener('canplaythrough',cacheFun3);
    videoElem3.addEventListener('ended',function () {
        g2Park7Out();
        g2Park8In();
        videoElem3.removeEventListener('waiting',videoWaiting)

    });
    videoElem3.addEventListener('waiting',videoWaiting)
    function cacheFun3(){
        go();
        $("#g2-park6 .question-last").on("click",function(){
            g2Park6Out()
            g2Park7In()
            videoElem3.play()

        })
        videoElem3.removeEventListener("canplaythrough",cacheFun3);
    }
    //第六步 e
    //第七步 s
    $("#g2-park8 .question-last").on("click",function(){
        var p8answer = $("[name='g2-park8Answer']").val();
        if(p8answer.length == 0){
            fbAlert("請输入答案");
            return false;
        }
        submit_content(2,p8answer);
        g2Park8Out();
        g2Park9In();

    })
    $("#g2-park8 .question-reset").on("click",function(){
        g2Park7In();
        g2Park8Out();
        videoElem3.play()
    })
    //第七步 e
    //结束
    $("#g2-park9 .question-reset").on("click",function(){
        g2Park1In();
        g2Park9Out();

    })
    $("#g2-park9 .question-last").on("click",function(){
        window.location.href="<?php echo e(route('pc.level.level_three')); ?>"

    })
</script>

