...
class AnswerController extends BaseController
{
    ...
    public function submitOption(Request $request)
    {
        ...
        $last_level_user_answer = UserAnswer::where('user_id',10)->where('level_id',$question->level_id)->orderBy('id','desc')->first();
        ...
        if(isset($last_level_user_answer) && $last_level_user_answer && $last_level_user_answer->question_id == $last_level_question->id)
        {
            ...
            $last_level_question_user_answer = UserAnswer::where('user_id',10)->where('level_id',$question->level_id)->where('question_id',$last_level_question->id)->orderBy('id','desc')->first();

            $user_answer = UserAnswer::where('user_id',10)->where('question_id',$question_id);

            $history_id = UserAnswer::where('user_id',10)->where('level_id',$question->level_id);
